...
async def get_token(form_data: OAuth2PasswordRequestForm = Depends()):
    """
    Bearer token - основной способ авторизации для использования REST API "Дневник мечты"

    Bearer token - используется для аунтификации и контроля доступа пользователя к данным сервиса,
    благодаря этому токену, можно отслеживать дельствия пользователей для дальнейшей аналитики.
    Он дает возможность "сохранять сессию пользователя" и обеспечивает удобный интерфейс для взаиможействия с сервисом.

    Токен можно получить при регистрации пользователя указав username и password
    или с использованием /users/token endpoint с аналогичными параметрами
    """
...
async def login(login_data: UserData):
    """
    Данный эндпоинт реализует повторную авторизацию пользователя
    """
